...
申請日を入力すると、**東京法務局・不動産（権利）登記**の完了予定日を自動表示し、案件を一覧管理するPWA。
完了予定日の当日以降になった案件は一覧で色付き表示される。
...
東京法務局の完了予定日ページ（不動産（権利）登記）を全件取得し、
`申請日 → 完了予定日` を庁ごとに `app/data/` へ書き出す。
...
- 不動産（表示）登記・商業登記は対象外（不動産（権利）登記のみ）

## 設計判断（合意済み）

- 形態：PWA（Mac不要・App Store不要・月額0円）
- 対象：東京法務局・不動産（権利）登記のみ、出張所は選択式
